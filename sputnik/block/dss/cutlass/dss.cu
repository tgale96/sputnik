...
using CanImplementFn = std::function<
    bool(const BlockMatrix, bool, const BlockMatrix, bool, Matrix)>;
...
static const bool k1 = RegisterKernel(can_launch_dss_mixed_b128_128x128x32x5_nt_align8,
...
cudaError_t Matmul(const BlockMatrix a, bool transpose_a,
  ...
  SPUTNIK_LOG(FATAL) << "No compatible kernel for dss problem.\n" << "m = " << shape.m <<
      "\nn = " << shape.n << "\nk = " << shape.k << "\nblock_size = " <<
      AsInt(b.block_size) << "\ntrans_a = " << transpose_a << "\ntrans_b = " <<
