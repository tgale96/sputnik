...
void BM_Dss(benchmark::State& state) {
  ...
  AllocateBitmaskBuffers(lhs);
  AllocateBitmaskBuffers(rhs);
  ...
  FreeBitmaskBuffers(lhs);
  FreeBitmaskBuffers(rhs);
...
BENCHMARK(BM_Dss)->Apply(BenchmarkArgs)->UseManualTime()->MinTime(1e-2);
